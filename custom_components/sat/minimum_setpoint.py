...
class MinimumSetpoint:
    ...
    def warming_up(self, return_temperature: float, boiler_temperature: float) -> None:
        if self.base_dt_temperature is not None and boiler_temperature > return_temperature:
            return

        # Use the new value if it's higher or none is set
        self.base_dt_temperature = boiler_temperature - return_temperature
        _LOGGER.debug(f"Higher temperature set to: {boiler_temperature - return_temperature}.")

        # Make sure to remember this value
        if self._store:
            self._store.async_delay_save(self._data_to_save)
            _LOGGER.debug("Stored base ΔT temperature changes.")
